...
export function CheckoutPage(){
  ...
  </form><CheckoutPreview items={items} subtotal={subtotal} fee={fee} total={total} fulfillment={form.fulfillment} selectedArea={selectedArea} vatRate={pricing.vatRate} pricesIncludeVat={pricing.pricesIncludeVat}/></div></main>
}
...
function CheckoutPreview({items,subtotal,fee,total,fulfillment,selectedArea,vatRate,pricesIncludeVat}){const {baseAmount,vatAmount}=vatBreakdownFromInclusiveAmount(subtotal,vatRate,pricesIncludeVat);return <aside className="checkout-preview"><header><span>Order preview</span><h2>Your order</h2></header><div className="checkout-preview-items">{items.map(item=><article key={item.lineId}><img src={item.image} alt=""/><div><h3>{item.quantity}× {item.name}</h3><p>{[item.variation?.name,item.temperature,item.ice,item.sugar].filter(Boolean).join(' · ')}</p>{item.addons?.length>0&&<small>{item.addons.map(addon=>addon.name).join(', ')}</small>}</div><b>{money((item.unitPrice+(item.addons||[]).reduce((sum,addon)=>sum+addon.price,0))*item.quantity)}</b></article>)}</div><div className="checkout-totals"><p><span>Subtotal</span><b>{money(baseAmount)}</b></p><p><span>{pricesIncludeVat?`VAT included (${formatVatRate(vatRate)})`:'VAT calculated at checkout'}</span><b>{money(vatAmount)}</b></p>{fulfillment==='delivery'&&<p><span>Delivery fee</span><b>{selectedArea?money(fee):'Select Barangay'}</b></p>}<p className="grand"><span>Total</span><b>{money(total)}</b></p></div></aside>}
