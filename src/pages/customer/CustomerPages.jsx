...
export function CheckoutPage(){
  ...
  const submit=async event=>{
    event.preventDefault();setSubmitError('');if(!/^\d{11}$/.test(form.contact)){setSubmitError('Contact number must contain exactly 11 digits.');return}if(form.fulfillment==='delivery'&&!/^\d{4,6}$/.test(form.postal)){setSubmitError('Postal code must contain 4 to 6 digits only.');return}if(form.fulfillment==='delivery'&&!selectedArea)return
...
function OrderCompleteModal({order,freshOrder=false,fallbackEstimatedTime='',onTrack,onContinue}){const [copied,setCopied]=useState(false);const orderNumber=order?.order_number||order?.reference_code||order?.order_id||order?.id;const displayOrderNumber=customerOrderNumber(orderNumber);const fulfillment=order?.order_type||order?.fulfillment||'delivery';const status=orderStatusLabel(order,{fresh:freshOrder});const paymentMethod=paymentMethodLabel(orderPaymentMethod(order));const paymentStatus=orderPaymentStatus(order);const itemCount=orderCount(order);const totalAmount=Number(order?.final_total??order?.total??0);const etaValue=fulfillment==='delivery'?(order?.estimatedDeliveryTime||fallbackEstimatedTime||orderScheduleLabel(order)):orderScheduleLabel(order);const shortAddress=fulfillment==='delivery'?shortenAddress(order?.delivery_address):'';const copyOrderNumber=async()=>{if(!displayOrderNumber||!navigator?.clipboard?.writeText)return;await navigator.clipboard.writeText(String(displayOrderNumber));setCopied(true);window.setTimeout(()=>setCopied(false),1400)};return <div className="payment-modal-backdrop"><section className="payment-modal order-complete-modal" role="dialog" aria-modal="true" aria-labelledby="complete-modal-title"><span className="complete-check"><Check/></span><span className="payment-modal-kicker">Order completed</span><h2 id="complete-modal-title">Your order has been placed.</h2><p>{completionMessage(order)}</p><div className="completion-summary"><div className="completion-summary-row"><span>Order number</span><div className="completion-summary-value completion-summary-inline"><b>{displayOrderNumber}</b><button className="completion-copy" type="button" onClick={copyOrderNumber}>{copied?'Copied':'Copy'}</button></div></div><div className="completion-summary-row"><span>Order status</span><div className="completion-summary-value"><b>{status}</b></div></div><div className="completion-summary-row"><span>Payment method</span><div className="completion-summary-value"><b>{paymentMethod}</b></div></div><div className="completion-summary-row"><span>Payment status</span><div className="completion-summary-value"><b>{paymentStatus}</b></div></div><div className="completion-summary-row"><span>Fulfillment</span><div className="completion-summary-value"><b>{fulfillmentLabel(fulfillment)}</b></div></div><div className="completion-summary-row"><span>Items</span><div className="completion-summary-value"><b>{itemCount} item{itemCount===1?'':'s'}</b></div></div><div className="completion-summary-row"><span>Total amount</span><div className="completion-summary-value"><b>{money(totalAmount)}</b></div></div><div className="completion-summary-row"><span>{estimatedTimeLabel(order)}</span><div className="completion-summary-value"><b>{etaValue}</b></div></div>{shortAddress&&<div className="completion-summary-row"><span>Delivery address</span><div className="completion-summary-value"><b>{shortAddress}</b></div></div>}</div><p className="completion-note">{completionNote(order)}</p><div className="payment-modal-actions completion-actions"><button className="secondary-button" type="button" onClick={onContinue}>Continue shopping</button><button className="primary-button" type="button" onClick={onTrack}>Track order</button></div></section></div>}
export function OrderReviewPage(){
  const {state}=useLocation();const navigate=useNavigate();const {signOut}=useAuth();const {items,subtotal,clearCart}=useCart();const [requestKey]=useState(()=>crypto.randomUUID());const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [modal,setModal]=useState(null);const [createdOrder,setCreatedOrder]=useState(null);const [freshOrder,setFreshOrder]=useState(false);const form=state?.checkout;
...
const canCustomerCancel=order=>CANCELLABLE_RAW_STATUSES.includes(String(order?.status||'').trim())
...
const refundStatusLabel=value=>value==='pending'?'Refund Pending':value==='processed'?'Refund Processed':value==='rejected'?'Refund Rejected':''
...
export function MyOrdersPage(){
  ...
  const runCancel=async(reason,notes)=>{
    ...
    try{
      ...
      patchOrder(order.id,{status:'Cancelled',cancellation_reason:reason,cancellation_notes:notes,refund_status:result.refund_status})
      setCancelOrderTarget(null)
      setToast(`${customerOrderNumber(order.order_number)} was cancelled.`)
    }catch(cause){
...
function CurrentOrderCard({order,addonNames,onView,onCancel,onTrack,index=0}){
  ...
  return <motion.article className={`current-order-card ${orderStatusTone(status)}`} {...cardEnter(index)}>
    <header>
      ...
      <span className={`status-chip ${orderStatusTone(status)}`}><StatusIcon status={status}/> {status}</span>
    </header>
    <p className="order-status-message">{STATUS_MESSAGE[status]||'Waiting for update'}</p>
    ...
    {!canCustomerCancel(order)&&<p className="order-cancel-hint">This order can no longer be cancelled because preparation may have already started.</p>}
  </motion.article>
}
...
function CancelOrderModal({order,onClose,onConfirm}){
  ...
  return <motion.div className="payment-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)onClose()}} {...backdropMotion}>
    <motion.section className="payment-modal" role="alertdialog" aria-modal="true" aria-labelledby="cancel-order-title" {...modalMotion}>
      ...
      <h2 id="cancel-order-title">Cancel {customerOrderNumber(order.order_number)}?</h2>
      ...
      <p className="payment-modal-warning">This cannot be undone. If you already paid, we'll start a refund review instead of assuming it's automatic.</p>
      ...
      <div className="payment-modal-actions">
        ...
        <button className="danger-button" type="button" onClick={submit} disabled={busy}>{busy?'Cancelling…':'Confirm Cancellation'}</button>
      </div>
    </motion.section>
  </motion.div>
}
