...
function CheckoutPreview({items,subtotal,fee,total,fulfillment,selectedArea}){return <aside className="checkout-preview"><header><span>Order preview</span><h2>Your order</h2></header><div className="checkout-preview-items">{items.map(item=><article key={item.lineId}><img src={item.image} alt=""/><div><h3>{item.quantity}× {item.name}</h3><p>{[item.variation?.name,item.temperature,item.ice,item.sugar].filter(Boolean).join(' · ')}</p>{item.addons?.length>0&&<small>{item.addons.map(addon=>addon.name).join(', ')}</small>}</div><b>{money((item.unitPrice+(item.addons||[]).reduce((sum,addon)=>sum+addon.price,0))*item.quantity)}</b></article>)}</div><div className="checkout-totals"><p><span>Subtotal</span><b>{money(subtotal)}</b></p>{fulfillment==='delivery'&&<p><span>Delivery fee</span><b>{selectedArea?money(fee):'Select Barangay'}</b></p>}<p className="grand"><span>Total</span><b>{money(total)}</b></p></div></aside>}function PaymentConfirmationModal({payment,total,busy,onClose,onConfirm}){const isCod=payment==='cod';const isBank=payment==='bank_transfer';const title=isCod?'Confirm Cash on Delivery':isBank?'Bank transfer instructions':'GCash payment instructions';const qr=isBank?'/assets/img/qr1.jpg':'/assets/img/qr.jpg';return <div className="payment-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)onClose()}}><section className="payment-modal order-flow-modal" role="dialog" aria-modal="true" aria-labelledby="payment-modal-title"><button className="payment-modal-close" type="button" onClick={onClose} disabled={busy} aria-label="Close payment dialog">×</button><span className="payment-modal-kicker">{isCod?'Before placing your order':'Complete your payment'}</span><h2 id="payment-modal-title">{title}</h2><div className="payment-modal-total"><span>Amount due</span><strong>{money(total)}</strong></div>{isCod?<><p>Your order will be paid when it arrives. Please confirm that you understand these rules:</p><ul><li>Cash on Delivery is available for delivery orders only.</li><li>COD is available for orders up to {money(1000)}.</li><li>Please prepare the exact amount whenever possible.</li><li>The order is still subject to store confirmation and availability.</li></ul>{total>1000&&<p className="payment-modal-warning">This order exceeds the COD limit. Go back and select GCash or Bank.</p>}</>:<div className="digital-payment-guide"><img src={qr} alt={`${isBank?'Bank':'GCash'} payment QR code`}/><div><p>{isBank?'Transfer the exact amount to the bank account represented by this QR code.':'Open GCash and scan the QR code.'}</p><ol><li>Send the exact total shown above.</li><li>Use your full name as the payment reference.</li><li>Save a clear screenshot or receipt after payment succeeds.</li><li>Continue to upload your proof of payment.</li></ol></div></div>}<div className="payment-modal-actions"><button className="secondary-button" type="button" onClick={onClose} disabled={busy}>Go back</button><button className="primary-button" type="button" onClick={onConfirm} disabled={busy||(isCod&&total>1000)}>{busy?'Placing order…':isCod?'Confirm COD order':'Continue to upload'}</button></div></section></div>}
