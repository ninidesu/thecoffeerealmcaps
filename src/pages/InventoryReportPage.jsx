...
import {
  AlertTriangle, ArrowDownToLine, ArrowUpFromLine, CalendarDays,
  ChevronLeft, ChevronRight, ClipboardList, Download, Eye, FileText, FilterX, SlidersHorizontal,
  PackageCheck, PackageX, Search, Settings2, Trash2, X,
} from 'lucide-react'
...
import { downloadInventoryMovementCsv, fetchInventoryReport } from '../services/inventoryReportService'
...
export default function InventoryReportPage() {
  ...
  const [filtersOpen, setFiltersOpen] = useManagementSessionState('admin:inventory-report:filter-row-open', true)
  ...
  return <AppShell role="admin" title="Inventory Report" onRefresh={load}>
    <section className="ir-range-bar report-filter-bar" aria-label="Inventory report date range"><div className="ir-range-label report-filter-label"><CalendarDays size={16} aria-hidden="true" /><span><b>Report range</b><small>{range.label}</small></span></div><div className="ir-presets report-filter-presets is-four" role="group" aria-label="Inventory report period">{PRESETS.map(([value, label]) => <button key={value} type="button" className={preset === value ? 'active' : ''} aria-pressed={preset === value} aria-expanded={value === 'custom' ? preset === value : undefined} aria-controls={value === 'custom' ? 'inventory-report-custom-range' : undefined} onClick={() => setPreset(value)}>{label}</button>)}</div><div className="report-filter-actions"><button type="button" className="report-filter-export" onClick={() => downloadInventoryMovementCsv(filteredRecords, range.label)} disabled={loading || !filteredRecords.length}><Download size={14} /> Export CSV</button><button type="button" className="ir-filter-toggle report-filter-toggle" aria-expanded={filtersOpen} aria-controls="inventory-report-extra-filters" onClick={() => setFiltersOpen((open) => !open)}><SlidersHorizontal size={15} /> {filtersOpen ? 'Hide filters' : 'More filters'}{activeFilterCount > 0 && <span>{activeFilterCount}</span>}</button></div></section>
    {preset === 'custom' && <div className="report-filter-popout ir-custom-popout" id="inventory-report-custom-range" role="group" aria-label="Custom inventory report date range"><div className="report-filter-popout-copy"><CalendarDays size={15} aria-hidden="true" /><span><b>Custom date range</b><small>Choose the start and end dates for this report.</small></span></div><div className="ir-custom-range report-filter-date-range"><label>From<input type="date" value={customFrom} max={customTo} onChange={(event) => setCustomFrom(event.target.value)} /></label><span>to</span><label>To<input type="date" value={customTo} min={customFrom} onChange={(event) => setCustomTo(event.target.value)} /></label></div></div>}
    {filtersOpen && <div className="ir-filter-row report-filter-fields" id="inventory-report-extra-filters" aria-label="Inventory report filters"><label>Movement<select value={movementType} onChange={(event) => setMovementType(event.target.value)}><option value="all">All movements</option>{Object.entries(MOVEMENT_META).map(([value, meta]) => <option key={value} value={value}>{meta.label}</option>)}</select></label><label>Inventory type<select value={itemType} onChange={(event) => setItemType(event.target.value)}><option value="all">All inventory</option><option value="ingredient">Ingredients</option><option value="finished_product">Finished products</option></select></label><label>Source<select value={source} onChange={(event) => setSource(event.target.value)}><option value="all">All sources</option><option value="order">Order-linked</option><option value="manual">Manual staff action</option><option value="system">System</option></select></label><label>Performed by<select value={staff} onChange={(event) => setStaff(event.target.value)}><option value="all">Everyone</option>{staffOptions.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div>}
