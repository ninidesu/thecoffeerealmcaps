import { BarChart3, Bell, Boxes, CheckCheck, ClipboardList, Coffee, FileBarChart, LayoutDashboard, LogOut, MenuSquare, Monitor, Moon, Package, ReceiptText, RefreshCw, Settings, ShieldCheck, Sun, Trash2, TrendingUp, Users, X } from 'lucide-react'
...
const staffGroups = [{ label:'', links:[['Order Preparation','/staff',ClipboardList],['Inventory Management','/staff/inventory',Boxes],['Supply Orders','/staff/supplies',Package],['Manage Menu','/staff/menu',Coffee],['Transactions','/staff/transactions',ReceiptText],['Settings','/staff/settings',Settings]] }]
...
export default function AppShell({ role, title, eyebrow, children, actions, titleActions, onRefresh, onNotifications, notificationCount = 0 }) {
  ...
  const staffDisplayName = profile?.full_name || profile?.username || profile?.email || user?.email || 'Staff member'
  const staffInitials = staffDisplayName
    .replace(/@.*$/, '')
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
  ...
  useEffect(() => {
    if (role !== 'staff' || !user?.id) return undefined
  ...
  useEffect(() => {
    if (role !== 'staff' || !user?.id || !isSupabaseConfigured) return undefined
    const add = (notification) => addStaffNotification(user.id, notification)
    const channel = supabase.channel(`staff-notification-center-${user.id}`)
    ...
    if (staffPreferences.notify_customer_cancellations) channel.on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders' }, ({ new: order, old }) => {
      if (order?.status !== 'Cancelled' || old?.status === 'Cancelled' || order.cancelled_by_role !== 'Customer') return
      const orderLabel = order.order_number || 'An order'
      const needsRefund = order.refund_status === 'pending'
      const reason = order.cancellation_reason ? ` Reason: ${order.cancellation_reason}.` : ''
      add({
        category: 'cancellations',
        title: needsRefund ? 'Cancellation and refund request' : 'Customer cancellation',
        message: `${orderLabel} was cancelled by the customer.${needsRefund ? ' A refund needs staff action.' : ''}${reason}`,
      })
    })
  ...
  const openNotifications = () => {
    ...
    else if (role === 'staff') setNotificationsOpen((current) => !current)
    ...
    <aside className="sidebar internal-sidebar">
      ...
      <div className="sidebar-footer-stack">
        <div className="sidebar-theme-switcher" role="group" aria-label="Theme options">
          {themeOptions.map(([value, label, Icon]) => <button key={value} type="button" className={preference === value ? 'active' : ''} aria-pressed={preference === value} title={label} onClick={() => setPreference(value)}><Icon size={18}/><span>{label}</span></button>)}
        </div>
        {role === 'staff' && <button type="button" className="sidebar-staff-profile" onClick={() => navigate('/staff/settings')} title={`Open profile for ${staffDisplayName}`} aria-label={`Open profile for ${staffDisplayName}, Operation Staff`}>
          <span className="sidebar-staff-avatar" aria-hidden="true">{staffInitials}</span>
          <span className="sidebar-staff-profile-copy"><strong>{staffDisplayName}</strong><small>Operation Staff</small></span>
        </button>}
        <button className="sidebar-exit" type="button" onClick={() => setLogoutOpen(true)}><LogOut size={19}/><span>Logout</span></button>
      </div>
    </aside>
    <main className="app-main internal-main"><header className={`page-header internal-page-header${eyebrow ? '' : ' is-compact'}`}><div><div className="internal-title-row"><h1>{title}</h1>{titleActions}</div>{eyebrow && <span>{eyebrow}</span>}</div><div className="header-actions"><div className="internal-utility-bar" aria-label="Workspace utilities"><div className="internal-live-datetime"><span>{new Intl.DateTimeFormat('en-PH', { weekday: 'short', month: 'short', day: 'numeric' }).format(now)}</span><b>{new Intl.DateTimeFormat('en-PH', { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true }).format(now)} PHT</b></div><div className="internal-notification-anchor" ref={notificationAnchorRef}><button type="button" className="internal-utility-button" aria-label={`Open notifications${visibleNotificationCount ? `, ${visibleNotificationCount} unread` : ''}`} aria-expanded={role === 'staff' ? notificationsOpen : undefined} aria-controls={role === 'staff' ? 'staff-notification-center' : undefined} title="Notifications" onClick={openNotifications}><Bell size={18} />{visibleNotificationCount > 0 && <span className="internal-utility-badge">{visibleNotificationCount > 99 ? '99+' : visibleNotificationCount}</span>}</button>{role === 'staff' && notificationsOpen && <aside className="staff-notification-center" id="staff-notification-center" role="dialog" aria-modal="false" aria-labelledby="staff-notification-title"><header><div><span>Notification center</span><h2 id="staff-notification-title">Recent activity</h2></div><button type="button" onClick={() => setNotificationsOpen(false)} aria-label="Close notifications"><X size={18} /></button></header><div className="staff-notification-actions"><button type="button" onClick={readAllNotifications} disabled={!unreadNotificationCount}><CheckCheck size={16} />Read all</button><button type="button" className="is-destructive" onClick={clearNotifications} disabled={!notifications.length}><Trash2 size={16} />Clear</button></div><div className="staff-notification-list">{notifications.length ? notifications.map((notification) => <button type="button" className={notification.read ? 'is-read' : 'is-unread'} data-category={notification.category} key={notification.id} onClick={() => readNotification(notification.id)}><i aria-hidden="true" /><span><b>{notification.title}</b><small>{notification.message}</small><time dateTime={notification.createdAt}>{notificationTime(notification.createdAt)}</time></span></button>) : <div className="staff-notification-empty"><Bell size={22} /><b>You’re all caught up</b><span>Operational alerts will stack here as they arrive.</span></div>}</div><footer><button type="button" onClick={() => { setNotificationsOpen(false); navigate('/staff/settings') }}>Notification settings</button></footer></aside>}</div><button type="button" className="internal-utility-button" aria-label="Refresh current page" title="Refresh" onClick={refreshPage}><RefreshCw size={18} /></button></div>{actions}</div></header>{children}</main>
