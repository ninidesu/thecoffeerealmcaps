...
export function CartProvider({ children }) {
 ...
 const addItem=(next)=>{setItems(current=>{const sig=signature(next);const found=current.find(i=>signature(i)===sig);return found?current.map(i=>signature(i)===sig?{...i,quantity:i.quantity+next.quantity}:i):[...current,{...next,lineId:crypto.randomUUID()}]});setDrawerOpen(true)}
 ...
 const value=useMemo(()=>({items,addItem,updateQuantity,removeItem,clearCart,itemCount,subtotal,drawerOpen,openCart,closeCart}),[items,itemCount,subtotal,drawerOpen])
