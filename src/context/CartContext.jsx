...
export function CartProvider({ children }) {
 ...
 useEffect(()=>{let active=true;setCheckingAvailability(true);fetchMenuCatalog().then(({products})=>{if(!active)return;const catalog=new Map(products.map(product=>[String(product.id),product]));setItems(current=>current.map(item=>{const product=catalog.get(String(item.productId));const available=Boolean(product?.available);const availabilityReason=available?'':product?'Currently out of stock':'No longer available';return item.available===available&&item.availabilityReason===availabilityReason?item:{...item,available,availabilityReason}}))}).catch(()=>{}).finally(()=>{if(active)setCheckingAvailability(false)});return()=>{active=false}},[user?.id])
