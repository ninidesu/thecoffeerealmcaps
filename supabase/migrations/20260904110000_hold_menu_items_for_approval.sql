-- Hide an existing item while its staff change is pending. Approval restores
-- and applies it; rejection restores the previous archive state.
alter table public.menu_change_approvals
  add column if not exists held_item_id uuid references public.menu_items(id),
  add column if not exists held_item_was_archived boolean;
...
declare v_id uuid; v_item_id uuid; v_was_archived boolean;
begin
  ...
  if p_action not in ('add','change','remove') or btrim(coalesce(p_item_name,'')) = '' then raise exception 'Invalid menu approval request'; end if;
  if p_operation not in ('upsert_menu_item','set_availability','archive_menu_item','duplicate_menu_item','upsert_main_category','upsert_subcategory','archive_main_category','archive_subcategory','bulk_availability') then raise exception 'Unsupported menu approval operation'; end if;

  if p_operation in ('upsert_menu_item','set_availability','archive_menu_item') then
  ...
  insert into public.menu_change_approvals(submitted_by, action, item_name, summary, change_types, operation, payload, held_item_id, held_item_was_archived)
  values (auth.uid(), p_action, left(btrim(p_item_name), 120), left(btrim(p_summary), 500), coalesce(p_change_types, '{}'), p_operation, coalesce(p_payload, '{}'::jsonb), v_item_id, v_was_archived)
...
begin
  ...
  if p_state = 'approved' then
    if v_request.operation = 'upsert_menu_item' then
      ...
      for v_item_id in select value::uuid from jsonb_array_elements_text(coalesce(v_request.payload->'ids','[]'::jsonb)) loop perform public.staff_set_menu_item_availability(v_item_id, v_available); end loop;
