-- Match the checkout formula: base price = menu price / 1.12, then 20% discount.
create or replace function public.create_customer_order_with_benefit_discount(request_payload jsonb)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  result jsonb; order_id uuid; benefit_kind text; target_item uuid; target_unit numeric(12,2); base_price numeric(12,2); discount numeric(12,2):=0; order_row public.orders%rowtype;
begin
  ...
  update public.orders set discount_type=case when benefit_kind='pwd' then 'PWD' else 'Senior' end,discount_customer_name=order_row.customer_name,discount_subtotal=base_price,discount_amount=discount,final_total=order_row.final_total-discount,updated_at=now() where id=order_id;
  update public.payments p set amount_due=p.amount_due-discount where p.order_id=order_id;
